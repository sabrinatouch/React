...
class Contact extends Component {
    ...
    handleInputChange(event) {
        // Retrieve the target input from the event that has just been passed in
        const target = event.target;

        // If the type is checkbox, then the value is set to the 'checked' type, otherwise ('?') the value is set to whatever the value of the target is (i.e. text, email)
        const value = target.type === 'checkbox' ? target.checked : target.value;

        // name will be any of the state fields defined above (firstname, lastname, etc.)
        const name = target.name;

        // Whatever field/name is being changed, the value's state is set to the new input
        this.setState({
            [name]: value
        });
    }
